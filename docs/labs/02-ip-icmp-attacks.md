...
1. **Fragmentation basics**  
   ...
   - Use Scapy to craft an IP packet to the client (`192.168.10.3`) that is fragmented into multiple pieces.  Send the fragments out of order and observe that the client reassembles them correctly.
...
3. **Smurf attack**  
   - Identify a broadcast address on the LAN (e.g. `192.168.10.255`).  Use Scapy to send ICMP echo requests to the broadcast address with the `client`’s IP as the spoofed source.  Observe the flood of echo replies sent to the client.  
   - Measure the amplification effect by counting the number of replies.

4. **ICMP redirect**  
   - In `router1`, temporarily enable sending of ICMP redirects (`sysctl -w net.ipv4.conf.all.send_redirects=1`).  
   - From the attacker, craft an ICMP redirect packet to the client, indicating that traffic to `192.168.30.0/24` should be sent via `192.168.10.2` (attacker).  Observe the client’s routing table (`ip route`).  
